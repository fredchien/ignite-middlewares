...
function checksCreateTodosUserAvailability(req, res, next) {
  ...
  return res.status(403).json({ error: 'Pro plan is not activated.' });
}
...
function checksTodoExists(req, res, next) {
  ...
  const valida = validate(id);

  const todo = userfind.todos.find((user) => user.id === id);

  if(!valida){
    return res.status(400).json({error: 'Id not uiid'});
  }
